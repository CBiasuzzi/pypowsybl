...
SeriesArray* createNetworkElementsSeriesArray(const JavaHandle& network, element_type elementType, const std::vector<std::string>& attributes) {
	ToCharPtrPtr attributesPtr(attributes);
    return new SeriesArray(callJava<array*>(::createNetworkElementsSeriesArray, network, elementType, attributesPtr.get(), attributes.size()));
}
